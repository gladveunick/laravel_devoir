...
class ProgrammeController extends Controller
{
    ...
    public function add()
    {
        ...
        return view('admin.programmes.add', compact('candidats'));
    }
   ...
   // Méthode store mise à jour pour gérer le téléchargement de fichiers PDF.
   public function store(Request $request)
{
    ...
    public function likeProgramme($id)
{
    $programme = Programme::findOrFail($id);
    $electeur = Electeur::find(auth()->id());

    // Vérifiez si l'utilisateur a déjà aimé ou n'a pas aimé ce programme
    $alreadyLiked = $programme->likes()->where('electeur_id', $electeur->id)->exists();
    $alreadyDisliked = $programme->dislikes()->where('electeur_id', $electeur->id)->exists();

    if (!$alreadyLiked && !$alreadyDisliked) {
        // Ajoutez le like
        $programme->electeurs()->attach($electeur->id, ['action' => 'like']);
    }
